 ...
 */
public class AvailableItemsToSellAdapter extends RealmBaseAdapter<AvailableItemToSell>{
    private LayoutInflater inflater;
    private RealmResults<AvailableItemToSell> mItems;

    public AvailableItemsToSellAdapter(Context context, RealmResults<AvailableItemToSell> realmResults, boolean automaticUpdate) {
    ...
    public View getView(int position, View view, ViewGroup viewGroup) {
        AvailableItemToSell itemToSell = mItems.get(position);
        View v = view;
        ...
        picture.setImageURI(Uri.parse(itemToSell.getPicture()));
        name.setText(itemToSell.getName());
        String strQuantity = "" + itemToSell.getQuantity();
        ...
        if(splitString.length == 2) {
            updateRealmResults(
                    Realm.getDefaultInstance()
                            .where(AvailableItemToSell.class)
